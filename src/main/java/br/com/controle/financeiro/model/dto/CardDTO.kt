...
class CardDTO : Serializable {
    ...
    var institution = UUID.randomUUID()

    constructor() : super() {}
    private constructor(id: UUID?, name: String?, number: String?, client: UUID?, institution: UUID?) : super() {
        this.id = id
        this.name = name
        this.number = number
        this.client = client
    ...
    companion object {
        fun fromCard(card: Card?): CardDTO {
            return CardDTO(
                card!!.id, card.name, card.number, card.responsible?.id,
                card.institution?.id
            )
        }
    }

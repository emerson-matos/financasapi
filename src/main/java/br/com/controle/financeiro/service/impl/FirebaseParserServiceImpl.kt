...
@Service
@FirebaseCondition
class FirebaseParserServiceImpl(private val firebaseAuth: AbstractFirebaseAuth) : FirebaseParserService {
    override fun parseToken(idToken: String): FirebaseTokenHolder {
        require(!idToken.isBlank()) { "FirebaseTokenBlank" }
        return try {
            val customToken = firebaseAuth.verifyIdToken(idToken)
            FirebaseTokenHolder(customToken)
        } catch (e: Exception) {

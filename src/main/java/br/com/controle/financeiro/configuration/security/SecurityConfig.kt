...
class SecurityConfig : GlobalMethodSecurityConfiguration() {
    object Roles {
        ...
        const val ROLE_ANONYMOUS = ROLE + ANONYMOUS
        const val ROLE_USER = ROLE + USER
        const val ROLE_ADMIN = ROLE + ADMIN
    }

    @Order(Ordered.HIGHEST_PRECEDENCE)
    @Configuration
    protected class AuthenticationSecurity(
        @param:Qualifier(value = UserServiceImpl.Companion.NAME) private val userService: UserDetailsService,
        private val firebaseProvider: FirebaseAuthenticationProvider
    ) : GlobalAuthenticationConfigurerAdapter() {
        @Value("\${br.com.controle.financeiro.firebase.enabled}")
        private val firebaseEnabled: Boolean? = null
        @Throws(Exception::class)
        override fun init(auth: AuthenticationManagerBuilder) {
            val encoder = PasswordEncoderFactories.createDelegatingPasswordEncoder()
            auth.userDetailsService(userService).passwordEncoder(encoder)
            if (java.lang.Boolean.TRUE == firebaseEnabled) {
    ...
    @Configuration
    protected class ApplicationSecurity : WebSecurityConfigurerAdapter() {
        @Value("\${br.com.controle.financeiro.firebase.enabled}")
        private val firebaseEnabled: Boolean? = null
        @Throws(Exception::class)
        ...
        companion object {
            private val ADMIN_ENDPOINT = arrayOf("/admin/**", "/actuator/**")
            private val OPEN_ENDPOINT = arrayOf("/api/open/**", "/**")
            private val USER_ENDPOINT = arrayOf(

...
class BankAccountController(
    ...
    fun newBankAccount(@RequestBody bankAccount: @Valid BankAccountDTO): EntityModel<BankAccountDTO> {
        ...
        val owner = userService.authenticatedUser
        val client = clientRepository.findByIdAndOwner(bankAccount.responsible, owner).orElseThrow()
        val institution = bankAccount.institution?.let { institutionRepository.findById(it).orElseThrow() }
    ...
    fun replaceBankAccount(
        ...
        val savedAccount = bankAccountRepository.findByIdAndOwner(id, owner).map { bankAccount: BankAccount ->
            ...
            //TODO extract to service
            val client = clientRepository.findByIdAndOwner(newBankAccountDTO.responsible, owner).orElseThrow()
            val institution = newBankAccountDTO.institution?.let { institutionRepository.findById(it).orElseThrow() }

...
class SingUpController {
    ...
    fun signUp(@RequestHeader(name = "X-Firebase-User") token: String) {
        require(!token.isBlank()) { "FirebaseTokenBlank" }
        val tokenHolder = firebaseService!!.parseToken(token)
        userService!!.registerUser(RegisterUserInit(tokenHolder?.uid, tokenHolder?.email, tokenHolder?.name))
    }
}
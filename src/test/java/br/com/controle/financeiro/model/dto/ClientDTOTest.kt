...
class ClientDTOTest {
    @Test
    fun testfromClientShouldReturnAValidClientDTO() {
        val client: ClientDTO
        val mock = Client(UUID.randomUUID(), "client", UserEntity())
        client = fromClient(mock)
        Assert.assertEquals(mock.id, client.id)
        Assert.assertEquals(mock.name, client.name)
    }

    @Test
    fun testToClientShouldReturnAValidClient() {
        val client: Client
        val mock = ClientDTO()
        mock.name = "client"
        mock.id = UUID.randomUUID()
        mock.owner = "owner"
        client = mock.toClient(UserEntity())
        Assert.assertEquals(mock.id, client.id)
        Assert.assertEquals(mock.name, client.name)
    }
}
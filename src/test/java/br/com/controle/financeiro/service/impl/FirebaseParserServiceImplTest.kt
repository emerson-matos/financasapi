...
class FirebaseParserServiceImplTest {
    @InjectMocks
    private val service: FirebaseParserServiceImpl? = null

    @Mock
    private val firebaseAuth: AbstractFirebaseAuth? = null
    ...
    fun parseNotBlankToken() {
        Mockito.doThrow(Mockito.mock(FirebaseAuthException::class.java)).`when`(firebaseAuth)
            .verifyIdToken(ArgumentMatchers.anyString())

...
class CardDTOResourceAssemblerTest {
    @InjectMocks
    private val resourceAssembler: CardDTOResourceAssembler? = null
    @Test
    fun testToResource() {
        val cardMock = CardDTO()
        cardMock.id = UUID.randomUUID()
        cardMock.name = "mock"
        ...
        Assert.assertNotNull(response.getLink("cards"))
        Assert.assertEquals(cardMock, response.content)
    }
}
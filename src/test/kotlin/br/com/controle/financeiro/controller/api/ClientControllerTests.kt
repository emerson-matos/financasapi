...
class ClientControllerTests : BaseModelTemplate() {
    @Autowired
    ...
    fun setup() {
        setupModel()
        Mockito.`when`<Any>(clientRepository!!.save(ArgumentMatchers.any())).thenReturn(client)
    ...
    fun clientPostTest() {
        mockMvc!!.perform(
            MockMvcRequestBuilders.post(API_CLIENT).contentType(MediaType.APPLICATION_JSON_UTF8)
                .content(CLIENT_JSON)
        )
    ...
    fun clientPutNewClientTest() {
        Mockito.`when`(clientRepository!!.findById(ArgumentMatchers.any())).thenReturn(Optional.of(client))
        mockMvc!!.perform(
            MockMvcRequestBuilders.put(API_CLIENT + "/{id}", UUID.randomUUID())
                .contentType(MediaType.APPLICATION_JSON_UTF8)
                .content(CLIENT_JSON)
    ...
    fun clientGetOneNotFoundTest() {
        mockMvc!!.perform(
            MockMvcRequestBuilders.get(API_CLIENT + "/{id}", UUID.randomUUID()).accept(MediaType.APPLICATION_JSON_UTF8)
        )
    ...
    fun clientDeleteNotFoundTest() {
        Mockito.doThrow(EmptyResultDataAccessException(1)).`when`(clientRepository).deleteById(ArgumentMatchers.any())
        mockMvc!!.perform(MockMvcRequestBuilders.delete(API_CLIENT + "/{id}", UUID.randomUUID()))
            .andExpect(MockMvcResultMatchers.status().isNoContent).andReturn()
    }

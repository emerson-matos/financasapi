...
class InstitutionDTOResourceAssemblerTest {
    @InjectMocks
    private val resourceAssembler: InstitutionDTOResourceAssembler? = null
    @Test
    fun testToResource() {
        val institutionMock = InstitutionDTO()
        institutionMock.id = UUID.randomUUID()
        institutionMock.name = "mock"
        val response = resourceAssembler!!.toModel(institutionMock)
        Assert.assertTrue(response.hasLinks())
        Assert.assertNotNull(response.getLink("self"))
        Assert.assertNotNull(response.getLink("institutions"))
        Assert.assertEquals(institutionMock, response.content)
    }
}
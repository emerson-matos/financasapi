...
class SingUpControllerTest {
    ...
    fun signUp() {
        val holder = Mockito.mock(FirebaseTokenHolder::class.java)
        Mockito.doReturn(holder).`when`(firebaseService).parseToken(ArgumentMatchers.anyString())
        Mockito.doReturn("some@one.tk").`when`(holder).email
        Mockito.doReturn("123456").`when`(holder).uid
    ...
    fun signUpWithBlackToken() {
        mockMvc!!.perform(MockMvcRequestBuilders.post("/api/open/signup").header("X-Firebase-User", ""))

...
class CardControllerTests : BaseModelTemplate() {
    @Autowired
    ...
    fun setup() {
        setupModel()
        Mockito.`when`<Any>(cardRepository!!.save(ArgumentMatchers.any())).thenReturn(card)
    ...
    fun cardPostTest() {
        mockMvc!!.perform(
            MockMvcRequestBuilders.post(API_CARD_URL).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
                .content(CARD_JSON)
    ...
    fun cardPutNewCardTest() {
        Mockito.`when`(cardRepository!!.findById(ArgumentMatchers.any())).thenReturn(Optional.of(card))
        mockMvc!!.perform(
            MockMvcRequestBuilders.put(API_CARD_URL + "/{id}", UUID.randomUUID())
                .contentType(MediaType.APPLICATION_JSON_UTF8).content(CARD_JSON)
        )
    ...
    fun cardGetOneFoundTest() {
        Mockito.`when`(
            cardRepository!!.findByIdAndOwner(

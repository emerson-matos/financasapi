...
class ClientDTOResourceAssemblerTest {
    @InjectMocks
    private val resourceAssembler: ClientDTOResourceAssembler? = null
    @Test
    fun testToResource() {
        val clientMock = ClientDTO()
        clientMock.id = UUID.randomUUID()
        clientMock.name = "mock"
        ...
        Assert.assertNotNull(response.getLink("clients"))
        Assert.assertEquals(clientMock, response.content)
    }
}
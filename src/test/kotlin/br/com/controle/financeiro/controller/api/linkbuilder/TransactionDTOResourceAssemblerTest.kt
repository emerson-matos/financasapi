...
class TransactionDTOResourceAssemblerTest {
    @InjectMocks
    private val resourceAssembler: TransactionDTOResourceAssembler? = null
    @Test
    fun testToResource() {
        val transactionMock = TransactionDTO()
        transactionMock.id = UUID.randomUUID()
        ...
        Assert.assertNotNull(response.getLink("transactions"))
        Assert.assertEquals(transactionMock, response.content)
    }
}
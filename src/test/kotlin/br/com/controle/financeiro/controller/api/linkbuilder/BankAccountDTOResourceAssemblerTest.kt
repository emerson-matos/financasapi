...
class BankAccountDTOResourceAssemblerTest {
    @InjectMocks
    private val resourceAssembler: BankAccountDTOResourceAssembler? = null
    @Test
    fun testToResource() {
        val bankAccountMock = BankAccountDTO()
        bankAccountMock.id = UUID.randomUUID()
        ...
        Assert.assertNotNull(response.getLink("bankaccounts"))
        Assert.assertEquals(bankAccountMock, response.content)
    }
}